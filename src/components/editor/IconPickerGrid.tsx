...
function IconGrid({
  ...
  // the scroll wrapper below) - labels need to flip to light text there too,
  // or they'd be unreadable against that same dark backdrop.
...
// The color-variant tabs + search + Social/Genéricos icon grids, shared by
// IconLibraryModal (its own standalone dialog, opened from ButtonEditDrawer/
// CardsInfoEditor) and LinkMediaPicker's merged icon-or-image panel, so both
// stay in sync with the same icon set, search, and curation behavior instead
// of drifting apart. Expects to sit inside a `flex flex-col` container with a
// bounded height (e.g. a Dialog's content area) - the grid below the search
// bar scrolls on its own via `min-h-0 flex-1`.
export function IconPickerGrid({ onSelect }: IconPickerGridProps) {
  ...
  return (
    <>
      ...
      {/* Plain scrolling div, not Radix's ScrollArea: nested in a flex
